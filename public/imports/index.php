...
<div class="dashboard">
    <aside class="sidebar" id="dashboard-sidebar">
        ...
        <nav class="nav" aria-label="Điều hướng chính">
            ...
            <a class="active" aria-current="page" href="imports/index.php"><span aria-hidden="true"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#import"></use></svg></span>Phiếu nhập kho</a>
    ...
    <div class="content-shell">
        <header class="topbar"><div class="topbar-left"><button class="menu-toggle" type="button" aria-controls="dashboard-sidebar" aria-expanded="false" aria-label="Mở menu"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#menu"></use></svg></button><div><span class="breadcrumb">Quản lý kho / Phiếu nhập kho</span><h1>Phiếu nhập kho</h1></div></div><div class="topbar-actions"><span><?= date('d/m/Y') ?></span><span class="top-avatar" aria-hidden="true">V</span></div></header>
        <main class="main-content" id="main-content" tabindex="-1">
            <div class="page-intro"><div><h2>Danh sách phiếu nhập kho</h2><p>Tra cứu hóa đơn và theo dõi hàng hóa nhập vào kho.</p></div><a class="import-btn primary" href="imports/create.php"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#file-plus"></use></svg> Tạo phiếu nhập</a></div>
            <?php if ($error === ''): ?>
            <section class="metrics import-metrics" aria-label="Thống kê theo bộ lọc hiện tại">
                <article class="metric"><div class="metric-top"><span>Phiếu nhập kho</span><i class="blue" aria-hidden="true"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#import"></use></svg></i></div><strong><?= number_format($total, 0, ',', '.') ?></strong><small>Theo bộ lọc hiện tại</small></article>
                <article class="metric"><div class="metric-top"><span>Tổng giá trị nhập (VNĐ)</span><i class="green" aria-hidden="true"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#wallet"></use></svg></i></div><strong><?= importEsc(formatVnAmount($amount)) ?></strong><small>Theo tổng tiền đã lưu trên phiếu</small></article>
                <article class="metric"><div class="metric-top"><span>Nhà cung cấp</span><i class="orange" aria-hidden="true"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#warehouse"></use></svg></i></div><strong><?= number_format($suppliers, 0, ',', '.') ?></strong><small>Theo tên trên các phiếu phù hợp</small></article>
            </section>
            <?php endif; ?>
            ...
            <section class="panel list-panel" aria-labelledby="list-title">
                ...
                <?php elseif (!$bills): ?><div class="empty-state"><span aria-hidden="true"><svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#file"></use></svg></span><h3><?= $where ? 'Không tìm thấy phiếu nhập phù hợp' : 'Chưa có phiếu nhập kho' ?></h3><p><?= $where ? 'Thử thay đổi từ khóa hoặc khoảng ngày để tìm lại.' : 'Tạo phiếu nhập đầu tiên để theo dõi hàng hóa vào kho.' ?></p><a class="import-btn secondary" href="<?= $where ? 'imports/index.php' : 'imports/create.php' ?>"><?= $where ? 'Xóa bộ lọc' : 'Tạo phiếu nhập' ?></a></div>
                <?php else: ?>
                <div class="import-table-wrap" tabindex="0" role="region" aria-label="Bảng phiếu nhập, có thể cuộn ngang">
                    ...
                    <tr><td class="invoice-cell" data-label="Số hóa đơn"><a class="invoice-number" href="imports/details.php?id=<?= (int) $bill['id'] ?>"><?= importEsc($bill['so_hoa_don']) ?></a><small><?= importEsc($bill['serial'] ?: 'Chưa có serial') ?></small></td><td class="supplier" data-label="Nhà cung cấp"><?= importEsc($bill['nha_cung_cap']) ?></td><td data-label="Kho nhập"><?= importEsc($bill['nhap_vao_don_vi']) ?></td><td class="nowrap" data-label="Ngày nhập"><?= $validDate($bill['ngay_nhap']) ? date('d/m/Y', strtotime($bill['ngay_nhap'])) : '—' ?></td><td class="numeric amount" data-label="Tổng tiền (VNĐ)"><?= importEsc(formatVnAmount($bill['tong_tien'])) ?></td><td class="numeric" data-label="Số mặt hàng"><?= (int) $bill['so_luong_mat_hang'] ?></td><td class="action-cell"><a class="detail-link" href="imports/details.php?id=<?= (int) $bill['id'] ?>" aria-label="<?= importEsc('Xem chi tiết phiếu ' . $bill['so_hoa_don']) ?>">Chi tiết <svg class="ui-icon" aria-hidden="true" focusable="false"><use href="assets/icons.svg#arrow-right"></use></svg></a></td></tr>
                    <?php endforeach; ?>
                    </tbody></table>
                </div>
